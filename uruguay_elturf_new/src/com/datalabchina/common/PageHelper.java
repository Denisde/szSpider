...
//	private static HttpHost proxy = new HttpHost("192.168.60.3", 8080);
...
//		return doGet(url, null, 0);
	...
	public String doGet(String url, String referer, int retry) {
		logger.info("doGet " + retry + " : " + url);
		HttpGet request = new HttpGet(url);
		request.setHeader("User-Agent","Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/46.0.2490.71 Safari/537.36");
		try {
			...
			try {
				...
				if (response.getStatusLine().getStatusCode() == 302) {
					...
					return doGet("http:"+response.getFirstHeader("Location").getValue(), "");
				}
				HttpEntity entity = response.getEntity();
				if (entity != null) {
...
//			}
	...
	// private static PageHelper setProxy(PageHelper http, Vector<String>vProxyInfo){
	// String sProxyURL = vProxyInfo.get(0).toString();
	// String sProxyPort = vProxyInfo.get(1).toString();
	// String sProxyUser = vProxyInfo.get(2).toString();
	// String sProxyPwd = vProxyInfo.get(3).toString();
	// if(sProxyURL.equals("")){
	// http.setProxyURL("");
	// return http;
	// }
	// if(!sProxyURL.equals("")&& !sProxyPort.equals("")){
	// http.setProxyURL(sProxyURL);
	// http.setProxyPort(Integer.parseInt(sProxyPort));
	// System.out.println("\nProxy************************************************************");
	// logger.info("Spider will use proxy "+sProxyURL+":"+sProxyPort);
	// // System.out.println("Spider will use proxy "+sProxyURL+":"+sProxyPort);
	// if(!sProxyUser.equals("") && !sProxyPwd.equals("")){
	// http.setProxyUser(sProxyUser);
	// http.setProxyPassword(sProxyPwd);
	// // http.setUser(sProxyUser);
	// // http.setPassword(sProxyPwd);
	// logger.info("Spider will use proxy by "+sProxyUser+"/"+sProxyPwd);
	// System.out.println("Spider will use proxy by "+sProxyUser+"/"+sProxyPwd);
	// }else{
	// logger.info("Spider will use proxy "+sProxyURL+":"+sProxyPort+" but not proxyusr and proxy pwd");
	// }
	// }
	// return http;
	// }
	...
	public boolean canOpenPage(String url,String referer){
		boolean flag = false;
		HttpGet request = new HttpGet(url);
			try {
				request.setConfig(config);
				...
				CloseableHttpResponse response = httpClient.execute(request,HTTP_CONTEXT);
				if(response.getStatusLine().getStatusCode()==200)flag=true;
		} catch (Exception e) {
			logger.error("",e);
		}
		return flag;
