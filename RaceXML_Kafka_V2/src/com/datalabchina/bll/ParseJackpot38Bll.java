...
public class ParseJackpot38Bll {
	private  Logger logger = Logger.getLogger(ParseJackpot38Bll.class.getName());
...
//		fileName ="C:\\Users\\zhida.li\\AppData\\Roaming\\Skype\\My Skype Received Files\\XML\\20160815\\XML\\38_MONTANT_TIRELIRE/77738289.xml";
				...
				while(reunionMatcher.find()){
					...
					while(courses.find())
					{
						...
						while(paris_course.find())
						{
							...
							String montant_tirelire = oCommonMethod.getValueByPatter(pari_course, "<montant_tirelire>(.*?)</montant_tirelire>");
							String code_pari = oCommonMethod.getValueByPatter(pari_course, "<code_pari>(.*?)</code_pari>");

//							montant_tirelire=montant_tirelire==null?null:montant_tirelire.replace(".", "").replace(",", ".");
//							String sSql ="";
							StringBuffer sSql =new StringBuffer(); 
						     ...
						     sSql=sSql.append(montant_tirelire==null?"NULL,":"N'"+montant_tirelire.replace(".", "").replace(",", ".")+"',");
						     ...
						     logger.info("pr_PmuInfoCentreKafka_XML_Jackpot_InsertData  " + sSql);	
						     oCommonDB.execStoredProcedures("pr_PmuInfoCentreKafka_XML_Jackpot_InsertData", sSql.toString());
						     sSql=null;
						}
						paris_course=null;
					}
